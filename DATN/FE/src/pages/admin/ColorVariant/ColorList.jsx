import {
  DeleteOutlined,
  EditOutlined,
  PlusCircleOutlined,
} from "@ant-design/icons";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import {
  Button,
  Input,
  message,
  Popconfirm,
  Space,
  Table,
  Tag,
  Skeleton,
} from "antd";
import React, { useState } from "react";
import { Drawer } from "antd";
import API from "../../../services/api";
import ColorEdit from "./ColorEdit";
import ColorAdd from "./ColorAdd";

const ColorList = () => {
  const queryClient = useQueryClient();
  const [searchTerm, setSearchTerm] = useState("");
  const [messageApi, contextHolder] = message.useMessage();
  const [isDrawerVisible, setIsDrawerVisible] = useState(false);
  const [currentColor, setCurrentColor] = useState();

  const { data, isLoading } = useQuery({
    queryKey: ["COLOR_KEY"],
    queryFn: async () => {
      const { data } = await API.get("/admin/variantcolor");
      return data.data.map((item, index) => ({
        ...item,
        key: item.id,
        stt: index + 1,
      }));
    },
  });

  const { mutate } = useMutation({
    mutationFn: async (id) => {
      await API.delete(`/admin/variantcolor/${id}/soft`);
    },
    onSuccess: () => {
      messageApi.success("Xóa màu sắc thành công");
      queryClient.invalidateQueries({ queryKey: ["COLOR_KEY"] });
    },
    onError: (error) => {
      messageApi.error("Có lỗi xảy ra: " + error.message);
    },
  });

  const columns = [
    { title: "#", dataIndex: "stt", key: "stt" },
    {
      title: "Tên màu",
      dataIndex: "value",
      key: "value",
    },
    {
      key: "action",
      align: "center",
      render: (_, item) => (
        <Space>
          <Popconfirm
            title="Xóa màu sắc"
            description="Bạn có chắc chắn muốn xóa không?"
            onConfirm={() => mutate(item.id)}
            okText="Có"
            cancelText="Không"
          >
            <Button danger size="small">
              <DeleteOutlined />
            </Button>
          </Popconfirm>
          <Button
            type="primary"
            onClick={() => {
              setCurrentColor(item);
              setIsDrawerVisible(true);
            }}
            size="small"
          >
            <EditOutlined />
          </Button>
        </Space>
      ),
    },
  ];

  const filteredData = data?.filter((color) => {
    const matchesName = color.value
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    return matchesName;
  });

  // Mở Drawer để thêm màu sắc mới
  const showDrawer = () => setIsDrawerVisible(true);

  // Đóng Drawer
  const onClose = () => {
    setCurrentColor(null); // Đặt lại khi đóng Drawer
    setIsDrawerVisible(false);
  };

  return (
    <div>
      {contextHolder}
      <div className="mb-5">
        <h1>Quản lý màu sắc</h1>
        <Button type="default" onClick={showDrawer}>
          <PlusCircleOutlined /> Thêm màu sắc
        </Button>
      </div>
      <Space style={{ marginBottom: 20 }}>
        <Input
          placeholder="Tìm kiếm màu sắc theo tên"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          style={{ width: 300 }}
        />
      </Space>
      <Skeleton loading={isLoading} active>
        <Table dataSource={filteredData} columns={columns} rowKey="id" />
      </Skeleton>
      {/* Drawer cho việc thêm màu sắc */}
      <Drawer
        title={currentColor ? "Cập nhật màu sắc" : "Thêm màu sắc"}
        width={500}
        placement="right"
        onClose={onClose}
        open={isDrawerVisible}
        style={{ padding: 0, height: "100%" }}
        styles={{ body: { padding: 20, height: "100%" } }}
      >
        <div style={{ height: "100%", overflowY: "auto", padding: "20px" }}>
          {currentColor ? <ColorEdit color={currentColor} /> : <ColorAdd />}
        </div>
      </Drawer>
    </div>
  );
};

export default ColorList;
