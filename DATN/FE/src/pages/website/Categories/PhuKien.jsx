...
const PhuKien = () => {
  ...
  const { data: products, isLoading } = useQuery({
    queryKey: ["PHUKIEN_PRODUCTS"],
    queryFn: async () => {
      const { data } = await API.get("/products");

      // Lọc sản phẩm thuộc danh mục
      return data.data
        .filter((item) => item.category?.name === "Phụ kiện")
        .map((item, index) => {
      ...
      <div style={{ backgroundColor: "#333", padding: "20px 0" }}>
        <section className="container mb-4">
          <div className="d-flex justify-content-between align-items-center">
            <h2 className="fs-2 fw-semibold text-light">Phụ kiện</h2>
          </div>
              ...
              <section className="p-3 bg-dark text-light rounded shadow-sm">
                ...
                <div className="mb-3">
                  ...
                  >
                    ...
                    <option value="<1tr">Dưới 1 triệu</option>
                    <option value="1tr-10tr">1 triệu - 10 triệu</option>
              ...
              <div className="row g-3">
                {filteredData.length === 0 && (
                  <div className="text-center text-light">
                    Không có sản phẩm Phụ kiện nào.
                  </div>
                )}
